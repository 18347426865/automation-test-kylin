...
def enable_message_notification(chat_name: str) -> None:
    """
    开启消息通知
     param1: 聊天名称
    """
    ...
    # 点击"开启消息通知"
    com.click_string_in_window_region(CN.listbar_region, '开启消息通知', fuzzy=True)
    time.sleep(2)

    print(f'{sys._getframe().f_code}  (*^▽^*)测试完成！\n')


def disable_message_notification(chat_name: str) -> None:
    """
    关闭消息通知
     param1: 聊天名称
    """
    ...
    # 点击"关闭消息通知"
    com.click_string_in_window_region(CN.listbar_region, '关闭消息通知', fuzzy=True)
    time.sleep(2)
...
if __name__ == '__main__':
    ...
    enable_message_notification(chat_name='测试专用')
    disable_message_notification(chat_name='测试专用')
