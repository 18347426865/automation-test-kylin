...
def uninstall_app(app: dict) -> None:
    """
    卸载应用
    Parameters:
    ...
    com.click_string_in_window('我的')
    time.sleep(1)

    com.click_string_in_window('应用卸载', fuzzy=True)
    time.sleep(1)

    com.click_string_in_window('请输入要搜索的应用卸载软件', fuzzy=True)
    time.sleep(1)
    ...
    try:
        com.click_string_in_window_region(region, string="卸载")
    except:
        assert res != {}, '未识别到卸载按钮, 可能是截图区域有问题'
